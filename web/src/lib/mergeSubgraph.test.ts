...
describe('mergeSubgraphs', () => {
  it('adds new neighbor nodes and edges without duplicating shared ones', () => {
    ...
    const merged = mergeSubgraphs(base, extra, 100).graph
  ...
  it('drops edges whose endpoint was not admitted', () => {
    ...
    const result = mergeSubgraphs(base, extra, 1)
    const merged = result.graph
  ...
  it('propagates truncation from either input', () => {
    expect(mergeSubgraphs(sub([node(1)], [], true), sub([node(2)], []), 100).graph.truncated).toBe(
      true,
    )
    expect(mergeSubgraphs(sub([node(1)], []), sub([node(2)], [], true), 100).graph.truncated).toBe(
      true,
    )
  })
